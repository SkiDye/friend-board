...
## v0.2.0 - Gallery UI Improvements (2025-11-10)
**갤러리 UI 전면 개편**

### 구조 변경
- **Before**: 리스트 형식
- **After**: 그리드 갤러리 (2/3/4열 반응형)
- **카드 스타일**: 정사각형 비율, DCInside 스타일
...
- **Grid Layout**: CSS Grid로 반응형 갤러리 구현

---

## v0.3.0 - Media Support (2025-11-10)
**동영상 및 다양한 미디어 지원**
...
## v0.4.0 - Community Features (2025-11-10)
**커뮤니티 기능 추가**
...
## v0.5.0 - Performance Optimization (2025-11-10)
**로딩 성능 최적화**
...
## v1.0.0 - Storage Migration (2025-11-11)
**Supabase Storage로 대전환**

### 구조 변경
- **Before**: Base64 → posts 테이블
- **After**: 파일 → Supabase Storage (1GB)
...
// After
{
  url: "https://.../storage/v1/object/public/media/...",
...
- **RLS (Row Level Security)**: 파일 접근 권한 관리
...
## v1.1.0 - UX Improvements (2025-11-11)
**사용자 경험 대폭 개선**
...
#### 2. BoardSkeleton 컴포넌트
- 게시판 그리드 레이아웃 미리 표시
- 8개 카드 스켈레톤
- 로딩 중임을 시각적으로 표현

#### 3. Storage 사용량 실시간 조회
...
## v1.2.0 - Optimistic UI Updates (2025-11-11)
**낙관적 UI 업데이트 도입**
...
onError: (err, vars, context) => {
  queryClient.setQueryData(['post', id], context.백업데이터)
}
```
...
### 사용된 기술
- **React Query Mutations**: onMutate, onError, onSettled
- **Optimistic Updates**: 낙관적 UI 패턴
- **Error Recovery**: 자동 롤백 메커니즘
...
## v1.3.0 - Patch Notes System (2025-11-11)
**패치노트 시스템 구축**
...
→ 스켈레톤 + 점진적 로딩으로 체감 속도 개선

### 3. Query Separation
→ 갤러리/상세보기 분리로 성능 최적화

### 4. Storage Migration
→ Base64 → Object Storage로 확장성 확보

### 5. Independent Metadata
→ 패치노트 등 메타데이터 분리로 유연성 확보
